...
def add_screenshot_to_pdf(pdf_bytes: bytes, link: str) -> bytes:
    ...
    try:
        # Scarica il logo (riutilizza la funzione esistente)
        logo_bytes = download_logo()
        logo_image = None
        if logo_bytes:
            try:
        ...
        screenshot_url = f"https://api.site-shot.com/?url={link}&userkey=MAAIEYKBJA7MPB7IUMXSH3QNVX&no_ads=1&delay_time=6000&no_cookie_popup=1&width=900&height=1200"
        ...
        print(f"Logo termina a y={page_height - margin - logo_height_pt:.1f}")
        ...
        print(f"ERRORE nell'aggiunta dello screenshot: {e}")
...
async def aggiungi_screenshot(request: Request):
    """
    Endpoint che riceve un PDF nel body e un link nell'header,
    cattura uno screenshot del link e lo aggiunge al PDF.
    """
    ...
    try:
        ...
        # Aggiungi screenshot al PDF
        final_pdf_bytes = add_screenshot_to_pdf(pdf_bytes, link)

        print("Screenshot aggiunto con successo!")
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=pdf_with_screenshot_{timestamp.replace(' ', '_').replace(':', '-')}.pdf"
            }
        )
